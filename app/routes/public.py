...
async def news_detail(news_id: int, request: Request, _db: DbSession, user: OptionalUser):
    if not settings.get("news_enabled"):
        return render(request, "errors/404.html", user=user, status_code=404)
...
async def events(request: Request, _db: DbSession, user: OptionalUser):
