...
def test_get_returns_default_when_not_set(app):
    ...
    assert settings.get("membership_year_start_month") == 3
    assert settings.get("membership_year_start_day") == 1
    assert settings.get("annual_membership_cost") == 10000
    assert settings.get("membership_shoots_included") == 20
    assert settings.get("additional_shoot_cost") == 500
...
def test_get_returns_stored_value(app):
    ...
    assert settings.get("membership_year_start_month") == 6
...
def test_set_and_get_roundtrip(app):
    ...
    settings.set("annual_membership_cost", 15000)
    assert settings.get("annual_membership_cost") == 15000
...
def test_set_overwrites_existing(app):
    ...
    settings.set("membership_year_start_month", 12)
    assert settings.get("membership_year_start_month") == 12
...
def test_get_all_includes_stored_values(app):
    ...
    settings.set("annual_membership_cost", 20000)
...
def test_save_many(app):
    ...
    assert settings.get("membership_year_start_month") == 9
    assert settings.get("membership_year_start_day") == 15
    assert settings.get("annual_membership_cost") == 20000
    assert settings.get("membership_shoots_included") == 30
    assert settings.get("additional_shoot_cost") == 1000


def test_bool_serialization(app):
    """Test that boolean settings serialize and deserialize correctly"""
    settings.set("news_enabled", True)
    assert settings.get("news_enabled") is True

    settings.set("news_enabled", False)
    assert settings.get("news_enabled") is False
...
def test_calculate_membership_expiry_before_year_start(app):
    ...
    expiry = settings.calculate_membership_expiry(start_date)
...
def test_calculate_membership_expiry_with_datetime(app):
    ...
    expiry = settings.calculate_membership_expiry(start_datetime)
