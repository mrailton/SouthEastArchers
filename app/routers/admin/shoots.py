...
from app.routers.admin._helpers import flash_form_errors, flash_service_warnings, require_perms
...
async def shoots_index(request: Request, db: DbSession, user: CurrentUser):
    ...
    return render(request, "admin/shoots.html", {"shoots": pagination.items, "pagination": pagination, "per_page": per_page}, user=user, db=db)
...
async def create_shoot_page(request: Request, db: DbSession, user: CurrentUser):
    ...
    return render(request, "admin/create_shoot.html", {"form": form, "active_members": active_members, "visitor_fee": visitor_fee}, user=user, db=db)
...
async def create_shoot_store(request: Request, db: DbSession, user: CurrentUser):
    ...
    active_members = ShootService.get_active_members_with_credits()
    visitor_fee = SettingsService.get("visitor_shoot_fee") / 100.0
    return render(request, "admin/create_shoot.html", {"form": form, "active_members": active_members, "visitor_fee": visitor_fee}, user=user, db=db, status_code=422)
...
async def edit_shoot_page(shoot_id: int, request: Request, db: DbSession, user: CurrentUser):
    ...
    active_members = ShootService.get_active_members_with_credits()
    visitor_fee = SettingsService.get("visitor_shoot_fee") / 100.0
    return render(request, "admin/edit_shoot.html", {"form": form, "shoot": shoot, "active_members": active_members, "visitor_fee": visitor_fee}, user=user, db=db)
...
async def edit_shoot_store(shoot_id: int, request: Request, db: DbSession, user: CurrentUser):
    ...
    active_members = ShootService.get_active_members_with_credits()
    visitor_fee = SettingsService.get("visitor_shoot_fee") / 100.0
    return render(request, "admin/edit_shoot.html", {"form": form, "shoot": shoot, "active_members": active_members, "visitor_fee": visitor_fee}, user=user, db=db, status_code=422)
