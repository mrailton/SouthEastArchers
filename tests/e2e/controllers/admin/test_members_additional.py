...
def test_activate_membership_service_failure(client, admin_user, test_user):
    ...
    with patch("app.services.MembershipService.activate_membership", return_value=ServiceResult.fail("Error reason")):
...
def test_activate_membership_with_payment_sends_receipt(client, admin_user, test_user):
    ...
    with patch("app.services.MembershipService.activate_membership", return_value=ServiceResult.ok(message="Activated")):
...
def test_create_member_service_error_shows_message(client, admin_user):
    ...
    with patch("app.services.UserService.create_member", return_value=ServiceResult.fail("already registered")):
