...
class MailService:
    ...
    def _send(
        ...
        **context: Any,
        ...
        try:
            ...
            html_body = MailService._render(html_template, **context)
            text_body = MailService._render(text_template, **context) if text_template else ""
    ...
    def send_password_reset(user_id: int, token: str) -> None:
        reset_url = MailService._safe_url_for("auth.reset_password", f"/auth/reset-password/{token}", token=token)
        MailService._send(
            user_id,
