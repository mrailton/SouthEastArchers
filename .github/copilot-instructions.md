...
- Forms use Pydantic schemas (`app/schemas/admin_forms.py`, `app/schemas/forms.py`); routes parse POST data with `parse_form()` from `app/schemas/form_helpers.py` and pass `FormView` to admin templates. Use `flash_field_errors()` from `app/templating.py` for validation flashes.
- Domain events emit from services; handlers run deferred after the HTTP response via `app/events/background.py`.
- Multi-entity writes use `BaseRepository.transaction()`; staged finance rows use `finance.add_transaction()`.
