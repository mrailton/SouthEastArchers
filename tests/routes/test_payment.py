...
@patch("app.routers.payment.SumUpService")
...
def test_complete_checkout_failed_status(mock_sumup_class, member_client):
    mock_sumup_class.return_value.get_checkout.return_value = Mock(
        status="FAILED", transaction_code=None, transaction_id=None
    )
    response = member_client.post("/payment/checkout/test_123/complete", follow_redirects=True)
...
def test_complete_checkout_membership_renewal(mock_email, mock_sumup_class, member_client, test_user):
    ...
    mock_sumup_class.return_value.get_checkout.return_value = Mock(
        status="PAID", transaction_code="TXN", transaction_id="txn_456"
    )
    set_session(
        member_client,
...
@patch("app.routers.payment.payment_service.initiate_membership_payment")
