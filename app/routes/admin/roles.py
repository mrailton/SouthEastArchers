...
async def create_role_page(request: Request, db: DbSession, user: CurrentUser):
    form = RoleForm()
    form.permissions.choices = [(p.id, p.name) for p in rbac.list_permissions()]
...
async def create_role_store(request: Request, db: DbSession, user: CurrentUser):
    ...
    form.permissions.choices = [(p.id, p.name) for p in rbac.list_permissions()]
    if form.validate():
        result = rbac.create_role(form.name.data, form.description.data, form.permissions.data or [])
        if result.success:
...
async def edit_role_store(role_id: int, request: Request, db: DbSession, user: CurrentUser):
    ...
    form.permissions.choices = [(p.id, p.name) for p in rbac.list_permissions()]
    if form.validate():
        result = rbac.update_role(role, form.name.data, form.description.data, form.permissions.data or [])
        if result.success:
