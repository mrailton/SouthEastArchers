...
class TestUserCreate:
    def test_create_member(self, runner):
    ...
    def test_create_member_without_phone(self, runner):
    ...
    def test_create_admin_user(self, runner):
    ...
    def test_create_admin_seeds_rbac_if_missing(self, runner):
...
class TestUserList:
    def test_list_no_users(self, runner):
        result = runner.invoke(args=["user", "list"])
        assert result.exit_code == 0
        assert "No users found" in result.output

    def test_list_shows_users(self, runner, test_user):
        result = runner.invoke(args=["user", "list"])
        assert result.exit_code == 0
        assert test_user.email in result.output
        assert test_user.name[:24] in result.output

    def test_list_shows_admin_status(self, runner, admin_user):
        result = runner.invoke(args=["user", "list"])
        assert result.exit_code == 0
        assert admin_user.email in result.output
        assert "Yes" in result.output

    def test_list_shows_membership_status(self, runner, test_user):
        """test_user has an active membership — the list should show it."""
        result = runner.invoke(args=["user", "list"])
        assert result.exit_code == 0
        assert "active" in result.output

    def test_list_shows_no_membership(self, runner, admin_user):
    ...
    def test_list_header_format(self, runner, test_user):
        result = runner.invoke(args=["user", "list"])
        assert result.exit_code == 0
...
class TestUserDelete:
    def test_delete_existing_user(self, runner, test_user):
    ...
    def test_delete_user_not_found(self, runner):
        result = runner.invoke(args=["user", "delete", "--id", "99999"])
        assert result.exit_code == 0
        assert "not found" in result.output

    def test_delete_cancelled(self, runner, test_user):
        user_id = test_user.id
