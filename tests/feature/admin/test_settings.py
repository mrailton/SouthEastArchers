...
def test_update_settings_failure_flash(admin_client):
    from unittest.mock import patch

    with patch("app.routes.admin.settings.settings.save_many", side_effect=RuntimeError("db error")):
        response = admin_client.post(
            "/admin/settings",
            data={
                "membership_year_start_month": "4",
    ...
    assert b"Error updating settings" in response.content
