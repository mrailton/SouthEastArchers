...
async def edit_transaction_store(transaction_id: int, request: Request, db: DbSession, user: CurrentUser):
    ...
    flash_form_errors(request, form)
    return render(request, "admin/edit_transaction.html", {"transaction": transaction, "form": form}, user=user, db=db, status_code=422)
...
async def delete_transaction(transaction_id: int, request: Request, db: DbSession, user: CurrentUser):
    ...
    flash(request, "success" if result.success else "error", result.message or ("Transaction deleted." if result.success else "Error deleting transaction."))
    return RedirectResponse(url="/admin/finance", status_code=303)
